...
def create(time, rdd):
    ...
    if rdd.count() > 0:
        ...
        data = rdd.filter(lambda x: len(x["extTweet"]["entities"]["hashtags"]) > 0)
        if (data.count() > 0):
            hashtags = data.map(lambda x: (x["extTweet"]['entities']['hashtags']))
        ...
        data = rdd.map(lambda x: x["extTweet"]["fullText"])
        ...
        data = rdd.map(lambda x: x["user"]["screenName"])
        df = spark.createDataFrame(data.map(lambda x: Row(name = x, timestamp = time)))
        df.createOrReplaceTempView("screenName")
        df = spark.sql("select name, count(*) as total, timestamp from screenName group by name, timestamp order by total desc limit 5")
        df.write.mode("append").saveAsTable("snTable")
        ...
        if data.count() > 0:
            ...
            df.show()
            df.write.mode("append").saveAsTable("ocurrencesTable")
        else:
            print("rdd is empty: no action")

if __name__ == "__main__":
    sc = SparkContext(appName="Projectc 2: sparkOperations")
    readtweets()
